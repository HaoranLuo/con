...
class GLMApiModel(VLModel):
    def __init__(
            ...
            top_p: float = 0.9,
            max_tokens: int = 256,
            base_url: Optional[str] = None,  # <--- 新增 base_url 参数
            **kwargs: Any,
    ) -> None:
        ...
        # 优先使用显式传入的 base_url，其次尝试从 kwargs 获取
        self.base_url = base_url or kwargs.get("base_url")

        from zhipuai import ZhipuAI

        # 初始化客户端：如果有 base_url 则传入
        if self.base_url:
